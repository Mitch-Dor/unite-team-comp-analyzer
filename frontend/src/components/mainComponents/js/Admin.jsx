...
import { fetchIDNameMapping, fetchTableDraftInformation, fetchTableInsights, fetchTableTraits, fetchAllTierListEntries } from './common/http.js';
...
function Traits() {
  ...
  const [new_level_strength_rows, set_new_level_strength_rows] = useState([blankLevelStrengthRow()]);
  ...
  function blankLevelStrengthRow() {
    return {
      pokemon_id: '',
  ...
  function submitLevelStrength(row) {
    console.log(row);
  }
  ...
  function postLevelStrength(row) {
    console.log(row);
  }
  ...
  function getNameFromID(pokemon_id) {
  ...
  function getAvailablePokemon(existing_data) {
    const used_ids = new Set(existing_data.map(r => r.pokemon_id));
  ...
  function Pokemon_Traits_Table_New() {
    const available_pokemon = getAvailablePokemon(pokemon_traits_table_data);
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  function Level_Strength_Table_New() {
    const available_pokemon = getAvailablePokemon(level_strengths_data);
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
        ...
        {new_level_strength_rows.map((row, idx) => (
          <tr key={idx}>
            <td>
              <select
                value={row.pokemon_id}
                onChange={(e) => {
                  const newValue = e.target.value === '' ? '' : Number(e.target.value);
                  set_new_level_strength_rows(prev =>
                    prev.map((r, i) => (i === idx ? { ...r, pokemon_id: newValue } : r))
                  );
                }}
              >
            ...
            <td>
              <div className="level-strength-cell">
                {row.level_strength.map((level_obj, level_idx) => (
                  <div key={level_idx} className="level-strength-row">
                    <label>Level {level_obj.level}:</label>
                    <input
                      type="text"
                        ...
                        set_new_level_strength_rows(prev =>
                          prev.map((r, i) =>
                            i === idx
                              ? {
                                  ...r,
                                  level_strength: r.level_strength.map((l, li) =>
                                    li === level_idx ? { ...l, explanation: newValue } : l
                                  ),
                                }
                              : r
            ...
            <td>
              <button className="admin-submit-button" onClick={() => {postLevelStrength(new_level_strength_rows[idx])}}>Submit Row</button>
            </td>
          </tr>
        ))}
      </table>
  ...
  function Pokemon_Insights_Table() {
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  function Pokemon_Insights_Table_New() {
    const available_pokemon = getAvailablePokemon(pokemon_insights_table_data);
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  function Tier_List_Table_New() {
    const available_pokemon = getAvailablePokemon(tier_list_table_data);
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  function Draft_Information_Table() {
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  function Draft_Information_Table_New() {
    const available_pokemon = getAvailablePokemon(pokemon_draft_information_table_data);
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Pokemon</th>
  ...
  ////////// POST ONLY SECTION
  //// playable_characters
  function Playable_Characters_Table_New() {
  ...
  //// pokemon_moves
  function Pokemon_Moves_Table_New() {
    return (
      <table className="admin-edit-table">
        <tr>
          <th>Move Name</th>
  ...
  return (
    <div id="admin-main-container">
      <div id="admin-mode-selector">
        ...
        <div className="admin-mode-selector" onClick={() => {set_current_state("POST_LEVEL_STRENGTH")}}>POST (new) Level Strength</div>
      ...
      <div id="admin-edit-section">
        ...
        {current_state === "POST_LEVEL_STRENGTH" && (
          <>
            <Level_Strength_Table_New />
            <button className="admin-add-row-button" onClick={() => {set_new_level_strength_rows(prev => [...prev, blankLevelStrengthRow()])}}>+ Add Row</button>
          </>
        )}
